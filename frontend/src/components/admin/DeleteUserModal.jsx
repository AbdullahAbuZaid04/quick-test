...
export default function DeleteUserModal({ isOpen, onClose, user, onDelete }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 overflow-y-auto bg-black/40 backdrop-blur-sm">
      ...
      <div className="relative bg-white w-full max-w-2xl rounded-2xl p-5 shadow-2xl my-auto">
        ...
        <div>
          <p className="text-lg">Are you sure you want to delete <span className="font-bold text-red-500">({user.name})</span>?</p>
        </div>
        ...
        <div className="flex justify-end gap-4 mt-8">
          ...
          <button onClick={() => onDelete(user.id)} className="px-8 py-3 bg-orange-500 hover:bg-orange-600 text-white font-bold rounded-2xl shadow-lg shadow-orange-100 transition-all duration-300">
            Delete User
          </button>
        </div>
      </div>
